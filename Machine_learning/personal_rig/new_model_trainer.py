...
## My model parameters ##
#########################
MODEL_VERSION = '0.4_multiclass'
VOCAB_SIZE = 7200
MAX_LEN = 250

OUTPUT_LAYER_DIM = 250
LSTM_LAYER_NODES = 256
...
def my_model():
    ...
    model.add(Dense(DENSE_LAYER_NODES,name='Forward_Comp_1', activation=DENSE_LAYER_ACTIVATION))
    model.add(Dropout(DROPOUT_CHANCE))

    model.add(Dense(DENSE_LAYER_NODES,name='Forward_Comp_2', activation=DENSE_LAYER_ACTIVATION))
    model.add(Dropout(DROPOUT_CHANCE))
...
# required for sparsecategoricalentropy
...
save_model_and_tok(MODEL_VERSION, model=model, token=tok)