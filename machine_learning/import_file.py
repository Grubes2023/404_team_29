...
### returns a list of the format [category_label, confidence_as_float_percentage]
def make_batch_pred(inp_tweets, model_v='0.8_' + 'charlie'):

    tweets_df = pd.DataFrame(inp_tweets, columns =['location', 'tweet', 'time'], dtype=object) 
    ################################
    ### LOAD MODEL and TOKENIZER ###
    ################################
    model, tok = load_model_and_tok(model_version=model_v)
...
# eventually will be how I send data to the user application
# data is of the format = [tweet, location, time, category(words), confidency]
def data_pipeline_to_app(data):
    print('This tweet should be passed along to the App')
    print(f'{data[0]}\nLocation: {data[1]}\nTime: {data[2]}\nCategory: {data[3]}\nConfidence : {data[4]} %\n')
    # pass

# eventually will be how I send data to the website
# data is of the format = [tweet, location, time, category(words), confidency]
def data_pipeline_to_website(data):
    print('This tweet should be passed along to the Website')
    print(f'{data[0]}\nLocation: {data[1]}\nCategory: {data[3]}\nConfidence : {data[4]} %\n')

# decides if a tweet moves on and passes along if it does\
# input is of the format model_prediction = [label, confidence]   and   input tweet = [location, tweet, time]
def prediction_decision(model_prediction, input_tweet):
    ...
    if ( (model_prediction[1] >= MIN_CONFIDENCE_LEVEL) and (model_prediction[0] != 0) ):  # confident that it is a category 1 or 2 (not 0)
        # format  [tweet, location, time, category(words), confidency]
        pass_along = [tweet, loc, time, categ, confidence]   
    ...
    # for testing purposes
        ...
        print('')
        print("This tweet would NOT have been passed along")
