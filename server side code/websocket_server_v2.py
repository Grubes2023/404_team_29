...
async def run_parser_and_model(zipcode):
    ...
    # let results be in the for [ {zipcode : [ [res1], [res2]]} ]

    twitter_response = await run_parser(zipcode)[0]
    ic(twitter_response)
    parser_results = twitter_response[zipcode]
    parser_end_time = time.time()
    parser_hits = len(parser_results)
    ...
    if parser_hits != 0:
        ...
        for i in range(0, parser_hits):        
            ...
            #curr_tweet = parser_results[i]
            #time_of_tweet = curr_tweet[i]
            ...
            # will need to adjust based on something
            # [ [tweet, zip, loc, time] , [], []]
...
async def command_execution(message, websocket):
    ...
    # decision-> manual update will update all connections associated with the user
        ...
        if client_info[curr_user][0] != '00000':
            ...
            info_to_send = await run_parser_and_model(zipcode_to_search)
...
# commands from the fake twitter (might remove [unnessesary])
async def handle_fake_twitter(websocket, path):
    try:
        async for message in websocket:
            executed_command = await command_execution(message, websocket)
            if not executed_command:
    ...
    # if connection is closed, remove websocket for twitter
    except websockets.exceptions.ConnectionClosed:
        del fake_twitter_connection_info['Connection']
        print("FAKE TWITTER disconnected")
        # websocket.close()
...
async def main(websocket, path):
    ...
    # this prioritizes fake twitter so it doesnt go through auth like a normal client
    if 'FAKE_TWITTER' in credentials:
        fake_twitter_connection_info['Connection'] = websocket
        pass
        # might not need below (i dont think there are any commands FROM the fake twitter)
        #await handle_fake_twitter(websocket, path)

    # authenticate user 
    elif await authenticate(websocket, credentials):
        username = credentials  # this is authentication message (initial message for each websocket)
