...
from confidential_information.confidential import get_local_ip, history_information
...
async def command_execution(message, websocket):
    ...
    if 'update' in command.lower():
        ...
        if zipcode.lower().strip() != 'undefined':
            return await get_zipcode_history(websocket, zipcode)
        else:
            print('Undefined Zipcode')
            return False
            #response = "Undefined Zipcode"
...
async def get_zipcode_history(websocket, zipcode):
    ...
    try:
        ...
        ic(update)
        json_data = json.dumps([update])
        curr_time = datetime.now().strftime("%m/%d/%Y, %I:%M:%S %p")
        ...
        await websocket.close()
        curr_time = datetime.now().strftime("%m/%d/%Y, %I:%M:%S %p")
        print(f"Client disconnected during History Update at {curr_time}\n")
        return False
