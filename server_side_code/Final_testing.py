...
versions = ['1.2','1.1_beta']
#current_testing_version = '1.1_alpha' #BASE_MODEL_VERSION + '_charlie'
...
def confusion_calculations(confusion_matrix, true_labels, predicted_labels, model_version):
    ...
    print(f'Recall : {round(modified_recall,1)} %')
    print(f'Precision : {round(modified_precision,1)} %')
    print(f'The Inner Accuracy (between 1 and 2) is : {round(inner_accuracy,1)} %')



for current_testing_version in versions:
    start = timer()
